...
otherwise, "Not like a palindrome"              
		...
		if(s.length()==3) {
			if(s.charAt(0)==s.charAt(2) && s.charAt(1)==s.charAt(1) && s.charAt(2)==s.charAt(0) ) {
				//burada char kullanildi cunku return yapiyor.
			...
			if(s.charAt(0)==s.charAt(s.length()-1)) {//ilk ve son karakter
